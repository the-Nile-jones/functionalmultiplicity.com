...
> **NOT fixed:** the two already-cached copies. Origin is clean (deployment URL 404s them; `?cb=` 404s). The edge object survives **everything**: `purge_everything` ×4, a targeted by-URL `files[]` purge, `development_mode: on`, a Cache Rule `(uri.path contains ".bak") -> cache:false` (ruleset `0f9a0f26…` v8, rule `96b96309…`), and `Cache-Control: no-cache` request headers. Every one returned `success: True`. `age` climbed monotonically through all of it (525 → 629 → 736 → 1588) — never evicted, never bypassed.
> **The tell:** the stuck object carries `Cache-Control: public, s-maxage=604800` — **a value `_headers` no longer sets anywhere.** It is a fossil from the pre-May-2026 config, i.e. the same species as the "May 2026 stuck-cache incident with s-maxage=604800" that the `_headers` comment already commemorates. **This has now happened twice. If it happens a third time, the answer is not a better purge — it is understanding why this zone's edge retains objects that every documented eviction path claims to have removed.**
> A `.bak` path with no pre-existing cached object (`/nonexistent-fake.bak-test`) correctly 404s, so rule + origin are fine; the failure is strictly eviction. Expect natural expiry ~2026-07-23 if nothing re-requests them.
> ⚠️ **The Cache Rule added for this is REDUNDANT and did not work.** Recommend removing it — `.assetsignore` is the real fix, and a permanent invisible non-functional zone rule is exactly what the 2026-04-14 note above warns against. Rollback: `scratchpad/cache-ruleset-pre-bakrule.json` (v7, 0 rules). Left in place pending Nile's call.
> 🔺 **`cf-cache-status` returned `DYNAMIC` while `age: 1588` was present on the same response** — a flat self-contradiction (a DYNAMIC response is uncached and cannot have an Age). Combined with the earlier findings, that header is now wrong in *every* state tested on this zone: while serving stale, while caching, and with development mode ON. **It carries no information here. Never cite it.**
